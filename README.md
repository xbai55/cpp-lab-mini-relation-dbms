...
Mini Relation DBMS 是一个轻量级关系型数据库管理系统，实现了接近 MySQL 基础交互方式的 SQL 命令行体验。

项目包含 SQL 解析、文件式表存储、基于 B+ 树的主键索引、TCP 客户端/服务端通信、命令行交互界面和自动化测试流程。系统以清晰的模块划分组织数据库核心能力，便于构建、运行、测试和扩展。
...
`tests/smoke_client_server.sh` 会自动启动服务端，通过客户端执行一组 SQL，并检查查询、更新和删除后的输出结果。

## 设计说明

项目使用自定义 `MiniVector` 模板管理动态序列，避免在核心数据结构中依赖 STL 标准容器。表名和列名应使用全英文小写字母，不包含下划线和特殊字符。字符串值需要使用双引号，例如 `"peter"`。
